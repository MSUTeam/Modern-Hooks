::Hooks.__Q <- {
	...
	function validateParameters( _q, _key, _oldInfos, _newInfos )
	{
		...
		else
		{
			...
			{
				::Hooks.error(format("Mod %s (%s) is wrapping function %s in bb class %s with more required parameters than before (used to be %i, wrapper returned function with %i)", _q.__Mod.getID(), _q.__Mod.getName(), _key, this.buildTargetString(_q), oldParams.len()-1, newParams.len()-1));
			}
